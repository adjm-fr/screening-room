...
def _isolate_display_names_catalog(tmp_path, mocker):
    ...
    mocker.patch("sources.streaming.PROVIDER_DISPLAY_NAMES_PATH", tmp_path / "provider_display_names.json")
...
def test_load_returns_empty_typed_frame_when_cache_missing(mocker):
    mocker.patch("sources.streaming.STREAMING_CACHE_PATH", Path("/nonexistent/streaming.parquet"))
