...
dataclass, defined in :mod:`utils.chat_state`) so the conversation persists
...
:mod:`utils.chat_prompt`. Both modules are re-exported here so existing
``from utils.chat import ...`` call sites keep working unchanged.
...
defined in :mod:`utils.chat_tools`). ``_run_tool`` and ``_render_tool_rows``
render Streamlit expanders (transport-with-UI), which is why they stay here
beside ``_ask_gemini`` rather than in the Streamlit-free ``utils.chat_tools``.
"""
...
from utils.allocine_search import search_theaters
from utils.chat_prompt import ChatContext, _streaming_context, build_chat_context, build_system_message
from utils.chat_state import CHAT_STATE_PATH, ChatState, chat_state, delete_chat_state, load_chat_state, save_chat_state
from utils.chat_tools import SHOWTIMES_TOOL, TASTE_TOOL, showtimes_query, top_matches
from utils.data_loader import _normalize_title
from utils.theater_manager import append_theater, load_theater_ids
from utils.ui import render_movie_card

# Re-exported for backward compatibility: these names moved to utils.chat_state
# / utils.chat_prompt during the chat.py module split (see CLAUDE.md's
# "cinema_dashboard architecture" section), but existing callers
# (pages/recommendations.py, utils/cmdk.py, tests/test_chat.py, tests/evals/)
# import them from utils.chat. Listed explicitly so ruff's F401 doesn't flag
# imports this module doesn't call itself — `__all__` membership counts as
# "used" for unused-import checks.
__all__ = [
    "CHAT_STATE_PATH",
    "ChatContext",
    "ChatState",
