...
- **Discover** — chip filters (genre, director multiselect with live search, min Letterboxd rating, min your-rating) over a poster rail of matching films, ranked by Letterboxd community rating (best first). The two rating sliders are independent: min Letterboxd rating narrows by community reception across the whole watchlist+ratings pool, while min your-rating (only films you've actually rated carry one) turns the tab into a rewatch/favorites filter. Each card shows your own star rating as a green chip (Letterboxd convention) next to the amber Letterboxd community average; both ratings are on the same 0–5 scale.
...
Power-user surface: prompt-suggestion chips, streaming spinner with transparent tool-call expanders, in-page pinned-recommendations column on the right, Markdown conversation export. Pins render as compact rows — thumbnail, title, director, and either the next screening or where the film streams — each linking to the film's detail page.

The pin picker offers every film the assistant named across the **whole conversation**, matched on both title spellings (whole-word, accent- and case-insensitive) against the same closed set the model draws from: what's screening *and* what's streaming. So a film recommended off the streaming block is pinnable even though it plays in no theater, and asking a follow-up question no longer un-offers the films from the reply before it.
