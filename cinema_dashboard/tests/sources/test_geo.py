"""Tests for sources.geo — geocoding cache logic with mocked Nominatim."""
...
def patched_cache_path(tmp_path: Path, mocker):
    """Redirect sources.geo.GEO_CACHE_PATH to a temp file so tests don't touch the real cache."""
    cache = tmp_path / "theaters_geo.parquet"
    mocker.patch("sources.geo.GEO_CACHE_PATH", cache)
    return cache
...
def test_geocode_cache_miss_calls_nominatim_and_writes_cache(theaters_csv, patched_cache_path, mocker):
    ...
    from sources.geo import load_geocoded_theaters
