...
| `TMDB_API_KEY` | No | — | TMDB API key for all 12 TMDB-sourced columns: `french_title`, `cast`, the crew columns (`directors`/`producers`/`writers`/`composers`), `trailer_url`, and the territory columns (`studio`/`country`/`origin_country`/`language`/`original_language`). Pipeline runs without it, but those columns stay `null` — that is three of the taste ranker's dimensions (directors, country, language), and a null `directors` additionally breaks the dashboard's showtimes matching |
...
contract now that `studio`/`country`/`language` are TMDB-sourced and seeded on every row; the two reads that reuse an existing cache to skip
...
**Territories & Provenance (TMDB, same request as the crew):**
- `studio` - Production companies (TMDB `production_companies`)
- `country` - Full co-production territory list, display names (TMDB `production_countries`)
- `origin_country` - Nationality of the production, **bare ISO 3166-1 codes** (TMDB `origin_country`) — a
  narrower list than `country`, not a re-spelling of it
- `language` - Spoken languages, English names (TMDB `spoken_languages`)
- `original_language` - ISO 639-1 code of the original language
- *(Any future Letterboxd detail type still expands into its own column automatically)*
...
6. **Rich Metadata Extraction** - Extracts comprehensive data from Letterboxd including:
   ...
   - **Territory columns** - `studio`, `country`, `origin_country`, `language` and `original_language` come from TMDB, not Letterboxd's details tab
...
7. **Flexible Detail Handling** - Uses `**details_by_type` to dynamically expand any Letterboxd detail type into its own column, so a new type is captured without code changes. The three TMDB owns (`studio`/`country`/`language`) are filtered out first — the expansion runs last and would otherwise overwrite the TMDB values (see [`CACHE_COLUMNS.md`](CACHE_COLUMNS.md#quirks-that-bite))
