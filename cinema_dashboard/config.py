...
class Settings(AppSettings):
    ...
    def streaming_service_slugs(self) -> set[str]:
        ...
        not gated by this set — see ``sources.streaming.STREAMING_COLUMNS``. The
        import is local (not a circular-import workaround — ``sources.streaming``
        never imports this module). ``config`` is imported very early and
        ...
        ``sources.streaming``.
        """
        from sources.streaming import _slugify
