...
fallback together. ``modules.config.settings`` is pointed at a throwaway parquet
...
from utils.taste import build_affinity
...
def app(mocker, movies_dir: Path) -> AppTest:
    ...
    mocker.patch("modules.config.settings.movies_output_path", movies_dir)
    mocker.patch("modules.config.settings.allocine_output_path", None)
...
def test_missing_data_renders_the_empty_state(mocker):
    mocker.patch("modules.config.settings.movies_output_path", None)
...
def test_detail_page_lists_screenings_with_a_one_click_ics(mocker, movies_dir: Path, tmp_path: Path):
    ...
    mocker.patch("modules.config.settings.movies_output_path", movies_dir)
    mocker.patch("modules.config.settings.allocine_output_path", showtimes)
...
def test_detail_page_survives_an_unreadable_showtimes_file(mocker, movies_dir: Path, tmp_path: Path):
    ...
    mocker.patch("modules.config.settings.movies_output_path", movies_dir)
    mocker.patch("modules.config.settings.allocine_output_path", broken)
