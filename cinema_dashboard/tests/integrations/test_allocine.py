...
def _mock_get(mocker, html: str):
    ...
    return mocker.patch("integrations.allocine.requests.get", return_value=mock_resp)
...
def test_get_paris_cinemas_paginates(mocker):
    ...
    mocker.patch("integrations.allocine.requests.get", side_effect=responses)
...
def test_search_returns_matching_cinema(mocker):
    mocker.patch(
        "integrations.allocine._get_paris_cinemas",
...
def test_search_caps_at_three_results(mocker):
    ...
    mocker.patch("integrations.allocine._get_paris_cinemas", return_value=cinemas)
