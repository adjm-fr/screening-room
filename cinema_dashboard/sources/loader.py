...
def build_taste_profile(ratings_df: pd.DataFrame) -> str:
    ...
    Thin formatter over the affinity profile in :mod:`core.taste` — favourite
...
def attach_streaming(df: pd.DataFrame, movies_output: str) -> pd.DataFrame:
    ...
    Adds one column per :data:`sources.streaming.STREAMING_COLUMNS` entry
    ...
    :func:`sources.streaming.load_streaming_providers` already caches the read
...
def coerce_str_list(value: object) -> list[str]:
    ...
    streaming badge renderer (:mod:`ui.cards`) and the calendar partition logic
