...
Two tools live here, beside ``utils.chat``'s ``search_theater``:
...
type lives in ``utils.chat``, which imports this module, so taking the frame
keeps this module import-cycle-free, Streamlit-free and directly unit-testable.
``utils.chat``'s dispatch passes ``ctx.wl_scored``.
...
def showtimes_query(
    ...
    *,
    ...
    case-folded, see :func:`utils.data_loader._normalize_title`) original *and*
