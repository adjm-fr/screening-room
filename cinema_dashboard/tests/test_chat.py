"""Tests for utils.chat — context builders, disk persistence and tool dispatch.
...
def fake_gemini(mocker):
    ...
    mocker.patch("utils.chat.st")  # the tool expanders need no real session here
    client = mocker.MagicMock()
    mocker.patch("utils.chat.genai.Client", return_value=client)
    return client
...
def test_ask_gemini_dispatches_new_tools(mocker, ctx, fake_gemini, tool_name, args, handler, expected_kwargs):
    spy = mocker.patch(f"utils.chat.{handler}", return_value=[{"title": "Ran"}])
...
def test_ask_gemini_search_theater_still_populates_pending(mocker, ctx, fake_gemini):
    ...
    mocker.patch("utils.chat.search_theaters", return_value=theaters)
    fake_gemini.models.generate_content_stream.side_effect = [
...
def test_ask_gemini_chains_two_tool_rounds(mocker, ctx, fake_gemini):
    top = mocker.patch("utils.chat.top_matches", return_value=[{"title": "Ran"}])
    shows = mocker.patch("utils.chat.showtimes_query", return_value=[{"title": "Ran"}])
    fake_gemini.models.generate_content_stream.side_effect = [
...
def test_ask_gemini_stops_after_the_round_budget(mocker, ctx, fake_gemini):
    spy = mocker.patch("utils.chat.top_matches", return_value=[])
